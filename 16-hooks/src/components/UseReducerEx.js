...
// - 업데이트 로직을 컴포넌트 외부로 빼놓음 > 코드 가독성 높아짐, 컴포넌트 외부에 작성한 코드를 다른 곳에도 적용시킬 수 있음
export default function UseReducerEx() {
    ...
    console.log('state 초기상태 > ', state);
    console.log('---------------- ');

    // #4. 액션핸들러 함수 정의
    // - 이 함수는 'dispatch'를 호출하여 액션을 발생시킴
    // - dispatch는 액션 객체를 인수로 받으며 {type : 'ACTION_TYPE', payload: data}형태
    // *payload
    // - 액션 객체에 담아 보내는 추가적인 데이터 (숫자, 문자열, 객체 등 모든 데이터 형태 가능)
