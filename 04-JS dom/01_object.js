// 변수 or 함수 매개변수가 전달될 때, 값 또는 참조가 어떻게 전달되는지?
...
let num = 1;
let num2 = num; // 값의 복사가 일어남 >>> num의 값을 num2로 복사
// 두 변수는 메모리에서 별도의 공간에 저장됨 >>> 서로 독립적으로 존재
console.log(`num: ${num}, num2: ${num2}`);
console.log(num === num2); // true; 엄격한 동등검사 // 값에 의한 전달이기 때문에 값을 비교
...
function changeValue(x) {
    x = 10; //매개변수 x에 10이라는 값을 할당
    console.log('x > ', x);
}
changeValue(num);
console.log('num > ', num); // 5; 함수 내에서 재할당 하더라도 해당 함수 스코프 내에서만 유효하고, 외부 값은 영향을 받지 않기 때문
...
// - 변수의 메모리 참조(위치)가 전달되므로, 함수 내에서의 요소를 변경하면 원본 변수도 변경됨.
...
// Q. 객체를 문자열 템플릿에서 읽기 좋게 출력하려면?
// A. JSON.stringify 라는 메소드를 사용함.
console.log(`obj: ${JSON.stringify(obj)}, obj2: ${JSON.stringify(obj2)}`);
// JSON.stringifysms "JS 객체 또는 값"을 JSON 형식의 문자열로 변환하는 메소드
...
console.log(obj3 === obj4); //false; 같은 값을 가지고 있다고 해도, 참조값이 다르다
// obj3과 obj4는 현재 데이터가 같지만, 서로 다른 별도의 객체이다
// 즉, 참조값(address, 주소)이 다르다.

obj3.five = 5; // obj3에 값 추가
console.log(obj3, obj4);
console.log(obj3 === obj4);
