...
const btn1 = document.querySelector('.btn--main');
const btn2 = document.querySelector('.btn--sub');
const btn3 = document.querySelector('.btn--point');
const btn4 = document.querySelector('.btn--disable');
const contaniner = document.getElementById('container');

// 1개 요소에 여러개의 addEventListener 등록 가능
...
btn1.addEventListener('mouseout', function () {
    btn1.style.backgroundColor = 'rgb(255, 47, 47)';
});
...
btn2.addEventListener('click', () => {
    ...
    div.innerHTML = 'Hi, from btn2';
    contaniner.append(div);
});

// 함수명을 매개 변수로 받아서 이벤트 실행
btn3.addEventListener('click', changeColor);

function changeColor() {
    const divs = document.querySelectorAll('#container div');
    console.log(divs); // 해당 요소들 전부 CSS 변화를 적용 시키기 위해 for..of 반복 사용
    for (let div of divs) {
...
function changeBtnColor() {
    console.log(this); // this, 자기 자신 > btn4
    console.log(this.parentNode); // this를 이용해 부모 요소에 접근
    this.style.backgroundColor = 'black';
}
// *this 키워드*
// - 이벤트가 일어난 DOM 요소를 가리킴
...
// - 브라우저는 발생한 이벤트에 대한 다양한 정보를 담은 "이벤트 객체(event objecy)"를 이벤트 리스너에게 전달한다.
// ex. mousedown 이벤트 발생 > 이벤트 객체는 (마우스 좌표, 버튼 번호) 정보를 가짐.
// ex. keydown 이벤트 발생 > 이벤트 객체는 (키 코드 값, 어떤 키가 눌렸는지에 대한 정보) 정보를 가짐.
